    ...
    S: EthSpec,
{
    ...
    fn get_finalized_update(&self, slot: u64) -> Result<Option<Update>> {
        let key: packed::Uint64 = slot.pack();
        self.get_cf(columns::COLUMN_FINALIZED_UPDATES, key.as_slice())?
            .map(|raw| Update::from_slice(&raw).map_err(Error::storage))
            .transpose()
    }
}
